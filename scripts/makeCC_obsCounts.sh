...
usage() { echo "
Usage: 
   $0 [options]* (-S <SAM_File> | [-m <int>] [-d <int>] -1 <m1> -2 <m2> -R <Ref_File> }
...
while getopts ":m:d:R:S:C:l:r:1:2:f:N:" o; do
    case "${o}" in
        m)
            mean=${OPTARG}
            ;;
		...
		r)
			...
			;;
		f)
			#file with transcripts frequencies
			trFreq_File=${OPTARG}
			;;
		1)
...
if [[ -z "${totalNReads}" ]];then
	#default value is 100M (for the whole genome)
	totalNReads=100000000;
fi
...
if [[ -z "${trFreq_File}" ]]; then
	echo -e "ERROR: Specify path to file with true frequencies using option -f \n"
	exit 1
fi
...
if [[ -z "${CC_Path}" ]]; then
	...
	start_time=`date +%s`
	${SCRIPT_DIR}/../lib/compute_obsRC_CC_Counts $SAM_File $trFreq_File $obs_pair1_file $totalNReads
	end_time=`date +%s`
