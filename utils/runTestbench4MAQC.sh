...
while getopts ":d:S:G:F:R:E:T:M:" o; do
    case "${o}" in
        d)
            Path2Dir=${OPTARG}
            ;;
...
cd ${Path2Dir}/results~
	...
	do
		...
		if [ $i == 0 ]; then
			...
			#1st get the bam file:
			samtools view -bT /import1/GENCODE_v19/new/gencode.v19.pc_transcripts.new.fa $SAM > subSample.bam
			wait
			samtools view -b -f 2 subSample.bam | samtools sort -n - subSample2
			wait
			samtools view -h subSample2.bam > subSample2.sam
			wait
			SAM=$PWD/subSample2.sam
			BAM=$PWD/subSample2.bam			
		fi
			...
			${SCRIPT_DIR}/../run-mcreg.sh -G $GTF_File -S $SAM -F $FA_File -R $REF_File -m $mean -d $deviation -l 50 > simreg.log
			...
			echo -e "\nCompute Correlation and MPE for SimReg"
			startTime=`date +%s`
			#Extract MAQC
			${SCRIPT_DIR}/extractMAQC_UHRR_4SimReg.sh $PWD/MCReg.iso.estimates
			wait
			#Compute Correlation
			sort ./MCReg.iso.estimates.MAQC_UHRR.txt | awk '{print $2}' > ./mcreg.iso.estimates_noNames
			...
			endTime=`date +%s`
			totalTimeSec=`expr $endTime - $startTime`
			echo "Done Statistics! Execution Time:"
			${SCRIPT_DIR}/seconds.sh $totalTimeSec
			echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
			echo ""
			...
			#Check any file inside de RSEM indexes folder
			if [ ! -f ${RSEM_Ref}.grp ];then
				echo "Missing RSEM indexes!"
				echo "Prepare RSEM indexes in $PWD ..."
				mkdir rsemIndex
			...
			/home/code/IMPORT/RSEM/rsem-1.2.8/rsem-calculate-expression --bam --paired-end $BAM $RSEM_Ref $PWD/rsem > rsem.log
			wait
			...
			echo "Compute Correlation and MPE for SimReg"
			startTime=`date +%s`
			#Extract NanoCounts
			${SCRIPT_DIR}/extractMAQC_UHRR_4RSEM.sh $PWD/rsem.isoforms.results
			wait
			...
			sort $PWD/rsem.isoforms.results.MAQC_UHRR.txt | awk '{print $2}' > ./rsem.isoforms.results.justFreq
			...
			echo "Compute Correlation and MPE for IsoEM"
			startTime=`date +%s`
			#Extract NanoCounts
			${SCRIPT_DIR}/extractMAQC_UHRR_4IsoEM.sh IsoEM.iso_estimates
			wait

			#IsoEM Correlation:
			#sort ./IsoEM.iso_estimates | awk '{print $2}' > ./IsoEM.iso_estimates.justFreq
			sort ./IsoEM.iso_estimates.MAQC_UHRR.txt | awk '{print $2}' > ./IsoEM.iso_estimates.justFreq
			...
			if [ $i != 0 ]; then
				echo "#Remove current File $SAM (occupies too much memory)"
				rm -rf $SAM
			fi
