...
readonly DEBUG=0
readonly code=~/code/wt

gtf=$1
option_freq=$2

genes_names=`${code}/gtf/get_genes.sh $gtf`
...
do
	...
	current_gene_freq=`grep $gene $genes_freq | awk '{print $2}'` 
	...
	tr=`grep $gene $gtf | awk -F";" '{ per[$2] += 1 } END { for (i in per) print i}' | sed -e 's/transcript_id//g' | sed -e 's/"//g' | sed -e 's/ //g'`
